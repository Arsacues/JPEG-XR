...
int jxr_write_image_bitstream(jxr_image_t image, FILE*fd)
{
    ...
    if (ALPHACHANNEL_FLAG(image)) {       
        ...
        if (image->window_extra_top || image->window_extra_right) {
            window_params[0] = 1;
            window_params[1] = (unsigned int)image->window_extra_top;
            window_params[2] = (unsigned int)image->window_extra_left;
            window_params[3] = (unsigned int)image->window_extra_bottom;
            window_params[4] = (unsigned int)image->window_extra_right;
        }
        else {
