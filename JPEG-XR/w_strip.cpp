...
/* Nima:	I need the array IN here after LBT */
	...
	if (cur_row >= -1 && cur_row < (height-1)) {
		...
		for (tx = 0; tx < (int) image->tile_columns ; tx += 1) {
			...
			for (mx = 0 ; mx < (int) image->tile_column_width[tx] ; mx += 1) {
				...
				//  LSB Embedding using MACROBLK_UP1_HP(image,ch,tx,mx,idx, 10) Array
				//	String dataToEmbed=101011;
				//	int stringbit[6] = {1,0,1,0,1,1};
				int sum=0;
				
				
			
				for(int mx_counter =0;mx_counter<2;mx_counter++){
					for(int idx_counter=0;idx_counter<2;idx_counter++){
						for(int block_counter=0;block_counter<2;block_counter++){
							

					if(rand()%2==0){
						if (MACROBLK_UP1_HP(image,0,tx,mx_counter,idx_counter,block_counter)%2==0){
						}else{
							MACROBLK_UP1_HP(image,0,tx,mx_counter,idx_counter,block_counter)+=1;
						}
					}
					else{
						if(MACROBLK_UP1_HP(image,0,tx,mx_counter,idx_counter,block_counter)%2==0){
				...
				sum=30*15*23;
					...
					//printf("Total Number of Information Embedded: %d\n",sum);
					
					
				
				    // block => 14  => Max 250 (as block value increase, distortion increases)
				    // block index (idx) 0->15 Max 32 (Row in Y axis)
				    //mx -> 31 (column in  x axes)
					//tx -> 11,12,14,18... other values of tx can not be touched!! 
					//6500
					//PSSNR
					//SSIM
					//Open CV
					...
					// 2 check HP coefficient value even or odd
				...
				//DETECTING
					...
					if(MACROBLK_UP1_HP(image,0,tx,mx_counter,idx_counter,block_counter)%2==0){
						...
						else if(MACROBLK_UP1_HP(image,0,tx,mx_counter,idx_counter,block_counter)%2==1);{
							printf(" 1 ");
							MACROBLK_UP1_HP(image,0,tx,mx_counter,idx_counter,block_counter)-=1;
						}
...
#define RIGHT_X(idx) ( idx == image->tile_column_width[tx] -1 )
	...
	static void first_prefilter422_up2(jxr_image_t image, int ch)
	{
		...
		for(tx = 0; tx < image->tile_columns; tx++)
		{
			...
			for (idx = 0 ; idx < (int)image->tile_column_width[tx] ; idx += 1) {
	...
	static void first_prefilter420_up2(jxr_image_t image, int ch)
	{
		...
		for(tx = 0; tx < (int)image->tile_columns; tx++)
		{
	...
	static void second_prefilter422_up1(jxr_image_t image, int ch)
	{
		...
		for(tx = 0; tx < image->tile_columns; tx++)
		{
			...
			for (idx = 0 ; idx < (int)image->tile_column_width[tx] ; idx += 1) {
				if(top_my < (int)EXTENDED_HEIGHT_BLOCKS(image) -1)
				{
					if ((tx == 0 && idx==0 && !image->disableTileOverlapFlag) ||
						(image->disableTileOverlapFlag && LEFT_X(idx) && !BOTTOM_Y(top_my))) {
							/* Across vertical blocks, left edge */
	...
	static void second_prefilter420_up1(jxr_image_t image, int ch)
	{
		...
		int ty = 0;
		/* Figure out which tile row the current strip of macroblocks belongs to. */
		while(top_my > (int)image->tile_row_position[ty]+image->tile_row_height[ty] - 1)
			ty++;
